...
$(document).ready(function () {
    ...
    tablaproducto = $('#tbProducto').DataTable({
        "ajax": {
            "url": $.MisUrls.url._ObtenerProductosPorTienda + "?IdBodega=0",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
    ...
    function buscarProducto() {
        if (parseInt($("#txtIdBodega").val()) == 0) {
            swal("Mensaje", "Debe seleccionar una Bodega primero", "warning")
            return;
        }
        tablaproducto.ajax.url($.MisUrls.url._ObtenerProductosPorTienda + "?IdBodega=" + $("#txtIdBodega").val()).load();
    ...
    $('#btnAgregarCompra').on('click', function () {
        ...
        if (!existe_codigo) {
            $("<tr>").append(
                $("<td>").append(
                    $("<button><i class='fa fa-trash'></i></button>").addClass("btn btn-danger btn-sm")
                ),
                $("<td>").append(
                    $("<button><i class='fas fa-pen'></i></button>").addClass("btn btn-primary btn-sm")
                        .data("idproducto", parseInt($("#txtIdProducto").val()))
                        .data("Ruc", parseInt($("#txtRucProveedor").val()))
                        .data("Codigo", String($("#txtCodigoProducto").val()))
                        .data("Nombre", String($("#txtNombreProducto").val()))
                        .data("Cantidad", parseInt($("#txtCantidadProducto").val()))
                        .data("PrecioCompra", parseInt($("#txtPrecioCompraProducto").val()))
                        .data("PrecioVenta", parseInt($("#txtPrecioVentaProducto").val()))
                ),
...
$('#tbCompra tbody').on('click', 'button[class="btn btn-primary btn-sm"]', function () {
    ...
    $("#txtCantidadProducto").val(Cantidad);
    $("#txtPrecioCompraProducto").val(PrecioCompra);
    ...
    $('#btnTerminarGuardarCompra').on('click', function () {
        ...
        compra = "<COMPRA>" +
            ...
            "<IdBodega>" + $("#txtIdBodega").val() + "</IdBodega>" +
